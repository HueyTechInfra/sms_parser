...
regex7 = r"Transaction of Rs.(\d+\.*\d+) has been made on .* credit card"
regex8 = r"Cash withdrawal .* Rs.(\d+\.*\d+) .* Debit card"
regex9 = r"Txn of Rs.(\d+\.*\d+) made on .* Debit card"
...
regex_kotak_insuff_bal = r".*cash withdrawal .* debit card .* could not be processed"
...
regex_otp3 = r".*requested a code to activate"
...
regex_kotak_cc = r".*Total Due on  Card.* (rs|inr) (\d+\.\d+). min amt"
...
def categoriser(data):
    for x in range(len(data)):
        ...
        # loan disbursed
        if re.search(regex_loan1, msg, re.IGNORECASE) or re.match(regex_loan2, msg, re.IGNORECASE):
            if re.search(regex_loan1, msg, re.IGNORECASE):
                match = re.search(regex_loan1, msg, re.IGNORECASE)
            else:
                match = re.match(regex_loan2, msg, re.IGNORECASE)
        ...
        # finding promotional/advertisement messages
            ...
            if(re.match(r".* credit card application",msg,re.IGNORECASE)):primary_type.append("info")
            else:primary_type.append("promotion")
            sub_category.append(None)
            amount.append(None)

        # finding emi msgs
        elif re.search(r".*EMI.*", msg, re.IGNORECASE) and not re.search(r".*credited",msg,re.IGNORECASE):
            ...
            if re.search(pattern_w3, msg, re.IGNORECASE) or re.search(r".* will be debited", msg, re.IGNORECASE):
                ...
                if re.search(r"EMI.*?Rs\.?\s*(\d+\.\d+).*?(due|earliest)", msg):
                    ...
                    amount.append(None)
            elif re.match(r".*transaction .* Rs (\d+\.*\d+) to EMI",msg,re.IGNORECASE):
                primary_type.append("transaction")
                match = re.match(r".*transaction .* Rs (\d+\.*\d+) to EMI",msg,re.IGNORECASE)
                amount.append(float(match.group(1)))
            # emi debit msg
            else:
                primary_type.append("transaction")
        ...
        elif re.match(pattern_w, msg) or re.match(pattern_w3, msg, re.IGNORECASE) or re.search(r".* will be debited", msg, re.IGNORECASE):
            primary_type.append("info")
            sub_category.append(None)
            amount.append(None)
        elif re.match(regex_kotak_insuff_bal,msg,re.IGNORECASE):
            primary_type.append("info")
            sub_category.append("insufficient balance")
            amount.append(None)

        elif re.match(regex_kotak_cc, msg, re.IGNORECASE):
            match = re.match(regex_kotak_cc, msg, re.IGNORECASE)
            primary_type.append("info")
            sub_category.append("credit card due")
            amount.append(match.group(2))

        # finding otp msgs
        elif re.search(regex_otp1, msg, re.IGNORECASE) or re.search(regex_otp2, msg, re.IGNORECASE) or re.search(regex_otp3,msg,re.IGNORECASE):
            primary_type.append("otp")
            amount.append(None)
            sub_category.append("otp")

        # finding standing instructions
        elif re.search(regex_auto_debit, msg, re.IGNORECASE) or re.match(r".* standing instruction",msg,re.IGNORECASE):
            primary_type.append("info")
        ...
        elif parse(pattern_temp, msg) or parse(pattern_temp1, msg):
            if parse(pattern_temp, msg):amount.append(float(parse(pattern_temp, msg)['amount']))
            else: amount.append(float(parse(pattern_temp1, msg)['amount']))
            primary_type.append("transaction")
        ...
        # finding credit msgs
            ...
            primary_type.append("transaction")
            if re.match(r".* salary",msg,re.IGNORECASE):sub_category.append("salary")
            else: sub_category.append("credit")
            amount.append(float(match.group(2)))
        ...
        # finding debit msgs
        elif re.search(regex4, msg) or re.search(regex6, msg) or re.search(regex5, msg):
        ...
        elif re.match(regex8, msg, re.IGNORECASE) or re.match(regex9, msg, re.IGNORECASE) or re.search(regex7, msg, re.IGNORECASE):
            if re.match(regex8, msg, re.IGNORECASE): match = re.match(regex8, msg, re.IGNORECASE)
            elif re.search(regex7, msg, re.IGNORECASE): match = re.search(regex7, msg, re.IGNORECASE)
            else: match = re.match(regex9, msg, re.IGNORECASE)
            amount.append(float(match.group(1)))
            primary_type.append("transaction")
            sub_category.append("debit")
        ...
        match_kotak = re.match(r"txn .* debit card .* at (\d+)",msg,re.IGNORECASE)
        ...
        if match_kotak:
            transaction_id.append(match_kotak.group(1))
        elif match3:
        ...
        match1 = re.search(r'\b(balance|bal|bal |credit limit).*(Rs.|INR |Rs. |INR)(\d+)', msg, re.IGNORECASE)
        match2 = re.search(r'\b(balance|bal|bal |credit limit).*(Rs. -|INR -)(\d+)', msg, re.IGNORECASE)
