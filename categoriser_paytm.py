...
def categoriser(data):
    for x in range(len(data)):
        ...
        # loan disbursed
        if re.search(regex_loan1, msg, re.IGNORECASE) or re.search(regex_loan2, msg, re.IGNORECASE):
        ...
        # e-commerce
        elif re.search(regex_sender, sender, re.IGNORECASE) and re.search(r"(order|placed|delivered|ordered|arriving|arrived|return)", msg,re.IGNORECASE):
            primary_type.append("info")
        ...
        pattern1 = r".*(A/c |A/c|A/C|A/C |a/c |ac |AC |Ac |account |account ending with)X*(\d+)"
        pattern8 = r".*(A/c |A/c|A/C|A/C |a/c |ac |AC |Ac |account |account ending with)(\d+X*\d+)"
        ...
        if match9:
            account_no.append(str(match9.group(2)[-4:]))
        elif match1 or match2:
            if match1: match = match1
            else: match = match2
            account_no.append(str(match.group(2)[-4:]))
        else:
